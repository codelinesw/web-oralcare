...
import BotonMenuSegmentos from '../images/BotonMenuSegmentos.png'
...
export default class Header extends React.Component {
  ...
  render() {
    const { showBotonMenu, showBotonMenuS } = this.props;
    return (
      <header className="main-navigation border-bottom bg-transparent border-0">
        ...
        {showBotonMenuS && <Link to="/imagecarousel" className="navbar-brand font-pacifico" style={{ position: 'absolute', top: '20px', right: '220px', zIndex: '200' }}>
          <img src={BotonMenuSegmentos} width="190" style={{ position: 'relative', top: '-15px' }} />
        </Link>}
      </header>
    );
  }
